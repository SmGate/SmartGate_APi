...
class EventController extends Controller
{
    ...
    public function addevent(Request $request)
    {
        ...
        if ($image != null) {
            ...
            $eventImages->image = $imageName;
        }
        $eventImages->image = $imageName ?? 'images/user.png';
        ...
        $mydata = [
            ...
            "notification" => [
                'title' => 'Up Coming Event', 'body' => $event->description,
        ...
        // var_dump($result);
        curl_close($ch);
        return response()->json(["data" => $event, "images" => $eventImages]);
    }
    ...
    public function events($userid)
    {
        ...
        $events = Event::where('userid', $userid)->orderBy('created_at', 'desc')->get();


        foreach ($events as $event) {
            $eventImages = EventImage::where('eventid', $event->id)->get();
